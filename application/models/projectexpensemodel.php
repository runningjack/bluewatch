...
class projectexpensemodel extends CI_Model
{
    ...
    public function getAllAmountByDate($start_date, $end_date)
    {
        $this->db->select_sum('amount');
        $this->db->where('director_status', 1);         
    ...
    public function getAmountAllByDate($start_date,$end_date)
    {
        $this->db->select_sum('amount');
        $this->db->where('director_status', 1);
        $this->db->where('log_date >=', $start_date);
    ...
    public function getExpByRevenueHeadProjectAmountByDate($id,$revenueHead,$start_date,$end_date)
    {
        $this->db->select_sum('amount'); 
        $this->db->where('expense_head', $revenueHead);
        $this->db->where('director_status', 1);
    ...
    public function toatalLogedHoursByDepartment($id,$department,$start_date,$end_date)
    {
    //  var_dump($id,$department);echo "<br/>";
        $date_from = date("Y-m-d", strtotime($start_date)) . ' 00:00:00';
        $date_to = date("Y-m-d", strtotime($end_date)) . ' 23:59:59';
        $this->db->select_sum('hours');
        $this->db->where('activities.project_id', $id);
        $this->db->where('employees.department', $department);
        // $this->db->where('activities.project_id', 8);
        // $this->db->where('employees.department',16);
       ...
       //$this->db->where('final_approved_by > ', 0);       
        $query = $this->db->get('activities');
        // var_dump('project_id', $id); 
        // var_dump('department', $department);
   //  var_dump($query->row_array()['hours']);exit();
        return $query->row_array();
    }


    public function totalpersonalCostbyDepartment($id,$department,$start_date,$end_date,$total_hour)
    {
    //  var_dump($id,$department,$start_date,$end_date,$total_hour);exit;
        $date_from = date("Y-m-d", strtotime($start_date)) . ' 00:00:00';
        $date_to = date("Y-m-d", strtotime($end_date)) . ' 23:59:59';
        //sum(activities.hours) * employee_rate.employee_rate
    //    $this->db->select_sum("(sum(activities.hours) * employee_rate.employee_rate)/".$total_hour);
        $this->db->select("((sum(activities.hours) * employee_rate.employee_rate)/$total_hour) AS employee_rate");

      //  $this->db->select("((sum(activities.hours) * employee_rate.employee_rate)) AS employee_rate");
        $this->db->select('activities.employee_id');
        $this->db->where('project_id', $id);
...
//var_dump($sum);exit;
    ...
    public function getExpByRevenueHeadDeptAmountByProjectByDate($project_id,$start_date,$end_date)
    {
        $this->db->select_sum('amount'); 
        $this->db->where('project_id', $project_id);
        $this->db->where('account_controller_status', 1);
        // $this->db->where('log_date >=', $start_date);
        // $this->db->where('log_date <=', $end_date);

        $date_from = date("Y-m-d", strtotime($start_date)) . ' 00:00:00';
        $date_to = date("Y-m-d", strtotime($end_date)) . ' 23:59:59';
        if ($start_date) $this->db->where("project_exp_log.log_date BETWEEN '$date_from' AND '$date_to'");                          
    ...
    //toatalLogedHoursByDepartment
    public function projectHourlyLogs($project_id,$start_date,$end_date)
    {
        $date_from = date("Y-m-d", strtotime($start_date)) . ' 00:00:00';
        $date_to = date("Y-m-d", strtotime($end_date)) . ' 23:59:59';
        $this->db->select("sum(activities.hours) as amount");
        $this->db->select('activities.employee_id');
        $this->db->where('project_id', $project_id);
        $this->db->where('project_manager_status', "Approved");
       ...
       return $query->row_array();
         ...
         //toatalLogedHoursByDepartment
    public function projectHourlyLogsNotValidated($project_id,$start_date,$end_date)
    {
        $date_from = date("Y-m-d", strtotime($start_date)) . ' 00:00:00';
        $date_to = date("Y-m-d", strtotime($end_date)) . ' 23:59:59';
        $this->db->select("sum(activities.hours) as amount");
    ...
    public function getPersonnelCostByDepartmentbyDate($dept,$start_date,$end_date)
    {
        $this->db->select_sum('rate_cost');
        $this->db->where('employees.department', $dept);
        $this->db->join('employees', 'employees.id = employee_hourly_cost_rate.employee_id', 'left');
    ...
    //SELECT SUM(`rate_cost`) AS rate_cost FROM (`employee_hourly_cost_rate`) LEFT JOIN `employees` ON `employees`.`id` = `employee_hourly_cost_rate`.`employee_id` WHERE `employees`.`department` = '1' AND `log_date` >= '2023-03-01' AND `log_date` <= '2023-05-31' AND `project_id` = '1'
    public function getPersonnelCostByDepartmentbyDateByPorjectbk($dept,$start_date,$end_date,$project_id)
    {
        $this->db->select_sum('rate_cost');
        $this->db->where('employees.department', $dept);
        $this->db->join('employees', 'employees.id = employee_hourly_cost_rate.employee_id', 'left');
    ...
    public function getPersonnelCostByDepartmentbyDateByPorject($dept,$start_date,$end_date,$project_id)
    {
        $this->db->select('SUM(employee_rate * hours) AS rate_cost');
        $this->db->where('employees.department', $dept);
          $this->db->join('employee_rate', 'employee_rate.employee_id = employee_daily_hours_vw.employee_id', 'left');
        $this->db->join('employees', 'employees.id = employee_rate.employee_id', 'left');
       
        $this->db->where('employee_daily_hours_vw.log_date >=', $start_date);
        $this->db->where('employee_daily_hours_vw.log_date <=', $end_date);
        $this->db->where('employee_daily_hours_vw.project_id', $project_id);
        $query = $this->db->get('employee_daily_hours_vw');
    ...
    public function getPersonnelCostByDate($id,$start_date,$end_date)
    {
        $this->db->select_sum('rate_cost');
        $this->db->where('project_id', $id);
        $this->db->where('log_date >=', $start_date);
    ...
    public function getAllPersonnelCostDate($start_date,$end_date)
    {
        $this->db->select_sum('rate_cost'); 
        $this->db->where('log_date >=', $start_date);
        $this->db->where('log_date <=', $end_date);
    ...
    public function getAllPersonnelCostByDate($start_date,$end_date)
    {
        $this->db->select_sum('rate_cost'); 
        $this->db->where('log_date >=', $start_date);
        $this->db->where('log_date <=', $end_date);
    ...
    public function getPersonnelCostAllByDate($start_date, $end_date)
    {
        $this->db->select_sum('rate_cost');
        $this->db->where('log_date >=', $start_date);
        $this->db->where('log_date <=', $end_date);
    ...
    public function getDirectorExpLog($limit, $start, $employid)
    {
       ...
       // $this->db->where('project_exp_log.asssigned_director', $employid);
        $this->db->order_by("project_exp_log.log_date desc");
        $query = $this->db->get('project_exp_log');
