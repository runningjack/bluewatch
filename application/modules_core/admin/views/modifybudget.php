                    ...
                    ?>
                      <div class="row form_field_outer_row list-group-item">
                        ...
                        <div class="form-group col-md-2">
                          ...
                          <?php echo form_dropdown('percentage[]', $percentageelement, $bd->percentage, 'onclick="calculateBudget(this)" required="" class="form-control budget_percent" id="percentage_' . $count . '"'); ?>
                        </div>
                        ...
                        <div class="form-group col-md-4">
                          <label> Amount </label>
                          <input type="text" class="form-control budget_shared w_90 " value="<?php echo $bd->department_budget; ?>" name="department_budget[]" id="department_budget_<?php echo $count ?>" placeholder="Budget % Amount">
                        </div>
                        ...
                        <div class="form-group col-md-6">
                          ...
                          <?php echo form_dropdown('department[]', $departmentelement, $bd->department, 'required="" class="department form-control" id="department_' . $count . '"'); ?>
                        </div>
                        <?php
                        $row_counter = 1;
                        foreach ($getRevenueHead as $head) {
                          //var_dump($budget_head_by_dept[$bd->department])
                        ?>

                          <div class="form-group col-md-2">
                            <label> <?php echo $head->revenue_head ?> </label>

                            <?php foreach ($budget_head_by_dept[$bd->department] as $allhead) {
                            ...
                            <?php echo form_dropdown(
                              'percentage_' . $head->revenue_head_id . '[]',
                              $percentageelement,
                              $current_percentage,


                              'onclick="calculateBudgetSubHeadSumModify(this)" required="" class="form-control budget_percent"
                            target_total_head="percentage_' . $count . '"
                            target_total_id="department_budget_' . $row_counter . '"
                            data-field_value_id="header_' . $head->revenue_head_id . '_' . $count . '"
                            field_value_id="header_' . $head->revenue_head_id . '_' . $count . '"
                            index = ' . $count . '
                            element_index  = ' . $count . '
                            department_budget  = department_budget_' . $count . '
                            ...
                            <input type="number" step="any" <?php foreach ($budget_head_by_dept[$bd->department] as $allhead) {
                                                              if ($allhead->budget_head == $head->revenue_head) { ?> value="<?php echo $allhead->budget_amount  ?>" <?php }
                                                                                ?> <?php } ?> class="form-control  w_90  budget_head_amount_1" name="<?php echo $head->revenue_head . '[]' ?>" id="header_<?php echo $head->revenue_head_id ?>_<?php echo $count?>" placeholder="<?php echo $head->revenue_head ?>" onchange="validateHeaderSum(this);" onkeyup="this.onchange();" onpaste="this.onchange();" oninput="this.onchange();">
                          </div>
